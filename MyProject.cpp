...
protected:
	...
	void localInit() {
		...
		for (int i = 0; i < 24; i++) {
			...
			ModelToLoad[i].init(this, SceneToLoad[i].ObjFile); //k
			TextureToLoad[i].init(this, SceneToLoad[i].TextureFile); //k
			DescriptorToLoad[i].init(this, &DSL1, {
						{0, UNIFORM, sizeof(UniformBufferObject), nullptr},
						{1, TEXTURE, 0, &TextureToLoad[i]}
				});
			if (strcmp(SceneToLoad[i].ObjFile, "models/doorText4.obj") != 0) {
				k++;
			}
		}
		printf("kappo %d \n", k);
	...
	void localCleanup() {
		...
		for (int i = 0; i < 24; i++) {
			...
			ModelToLoad[i].cleanup();//k
			TextureToLoad[i].cleanup();//k
			if (strcmp(SceneToLoad[i].ObjFile, "models/doorText4.obj") != 0) {
				k++;
			}
		}
	...
	void populateCommandBuffer(VkCommandBuffer commandBuffer, int currentImage) {
		...
		int k = 0;
		for (int i = 0; i < 24; i++) {
			VkBuffer vertexBuffers[] = { ModelToLoad[i].vertexBuffer }; //k
			VkDeviceSize offsets[] = { 0 };
			vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
			vkCmdBindIndexBuffer(commandBuffer, ModelToLoad[i].indexBuffer, 0,    //k
			...
			vkCmdDrawIndexed(commandBuffer,
				static_cast<uint32_t>(ModelToLoad[i].indices.size()), 1, 0, 0, 0);  //k
			if (strcmp(SceneToLoad[i].ObjFile, "models/doorText4.obj") != 0) {
				k++;
			}
		}
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		static glm::vec3 RobotPos = glm::vec3(4, 0, 1);//4,0,1   1,0,3
		glm::vec3 RobotCamDeltaPos = glm::vec3(0.0f, 0.335f, -0.0f);
		static float lookYaw = 0.0;
		static float lookPitch = 270.0; // 0 
		static float lookRoll = 0.0; 
		static glm::vec4 blockLimit = glm::vec4(4.5, 3.5, 1.5, 0.5); //4.5, 3.5, 1.5, 0.5    1.5, 0.5, 3.5, 2.5
		static glm::vec4 recentlyWalls = glm::vec4(0, 0, 0, 0);//destra, sinistra, sotto, sopra
		static int matrixCoo[2] = { 10, 10 };//10,10   7,12
		...
		{
			matrixCoo[0]++;
			//printf("la mia coordinata x %f ", matrixCoo[0]);
			...
			if (vettore[matrixCoo[1]][matrixCoo[0]] == "*" || blockLimit[0] == 3.5 && RobotPos.z <= 3.4 && RobotPos.z >= 2.6 && dCheck1 == false || blockLimit[0] == 6.5 && RobotPos.z <= 8.5 && RobotPos.z >= 7.6 && dCheck2 == false)
			{
				...
				if (vettore[matrixCoo[1]][matrixCoo[0] + 1] == "*")
				{
					...
					blockLimit[0]++;
					//printf("aumento normale %f ", blockLimit[0]);
		...
		for (int i = 0; i < 24; i++) {
			...
			if (i == 19) {
				if (dCheck1) {
					ubo.model = glm::translate(glm::mat4(1.0), glm::vec3(0, 1, 0))*glm::translate(glm::mat4(1.0), glm::vec3(2.5, 0.8, -2.2))*glm::translate(glm::mat4(1.0), glm::vec3(-12.4, -0.8, -3.9));
				}
			}
			if (i == 20) {
				if (dCheck3) {
